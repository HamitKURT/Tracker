...
def wait_for_write_alias(es):
    """Wait until the write alias is available (created by kibana_deployer)."""
    while True:
        try:
            if es.indices.exists_alias(name=INDEX):
                logger.info(f"Write alias '{INDEX}' is available.")
                return
            logger.info(f"Waiting for write alias '{INDEX}' to be created by deployer...")
        except Exception as e:
            logger.warning(f"Error checking alias: {e}")
        time.sleep(5)


def process_logs():
    r, es = connect_services()
    wait_for_write_alias(es)
